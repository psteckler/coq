...
struct
  ...
  *)
  ...
  let rec strip_app_ORIG s =
    let rec aux_ORIG out arg = 
      match arg with
      | ( App _ | Shift _ as e) :: s -> aux (e :: out) s
      | s -> List.rev out,s
    and aux out arg = 
      let name = "aux in strip_app" in
      let _ = Timer.start_timer name in
      try
	let result = aux_ORIG out arg in
  ...
  let rec zip_ORIG ?(refold=false) = function
    | f, [] -> f
    | f, (App (i,a,j) :: s) ->
  ...
  and zip ?(refold=false) (f,t) = 
    let name = "Stack.zip" in
    let _ = Timer.start_timer name in
    try
      let result = zip_ORIG ~refold (f,t) in
      let _ = Timer.stop_timer name in 
      result 
    with
      exn -> 
...
let rec reducible_mind_case_ORIG c = 
...
let rec reduce_and_refold_fix_ORIG recfun env cst_l fix sk =
  ...
  apply_subst
    ...
    [] Cst_stack.empty raw_answer sk
and reduce_and_refold_fix recfun env cst_l fix sk =
  let name = "reduce_and_refold_fix" in
  let _ = Timer.start_timer name in
  try
    let result = reduce_and_refold_fix_ORIG recfun env cst_l fix sk in
    let _ = Timer.stop_timer name in
    result
  with exn ->
    let _ = Timer.stop_timer name in
...
let rec whd_state_gen_ORIG ?csts tactic_mode flags env sigma =
  let rec whrec_ORIG cst_l (x, stack as s) =
    ...
    | LetIn (_,b,_,c) when Closure.RedFlags.red_set flags Closure.RedFlags.fZETA -> (
      let name = "whrec LetIn" in
      let _ = Timer.start_timer name in
      try 
	let result = apply_subst whrec [b] cst_l c stack
	in
	let _ = Timer.stop_timer name in
	result
      with exn ->
    ...
    | Rel _ | Var _ | Const _ | LetIn _ | Proj _ -> (
      let name = "whrec Rel|Var|Const|LetIn|Proj" in
      let _ = Timer.start_timer name in
      try 
	let result =
	  fold ()
    ...
    | Sort _ | Ind _ | Prod _ -> (
      let name = "whrec Sort|Ind|Prod" in
      let _ = Timer.start_timer name in
      try 
	let result =
	  fold ()
...
let rec whd_betaiota_deltazeta_for_iota_state_ORIG ts env sigma csts s =
  let rec whrec_ORIG csts s =
    ...
    |args, (Stack.Case _ :: _ as stack') -> (
      let name = "whd_bi_dz_fis BRANCH 1" in
      let _ = Timer.start_timer name in
      try
	let result = 
	  let (t_o,stack_o),csts_o = whd_state_gen ~csts:csts' false
	    (Closure.RedFlags.red_add_transparent betadeltaiota ts) env sigma (t,args) 
	  in
	  if reducible_mind_case t_o then 
	    whrec csts_o (t_o, stack_o@stack') else 
	    s,csts'
	in 
	let _ = Timer.stop_timer name in
	result
    ...
    |args, (Stack.Fix _ :: _ as stack') -> (
      let name = "whd_bi_dz_fis BRANCH 2" in
      let _ = Timer.start_timer name in
      try 
	let result = let (t_o,stack_o),csts_o = whd_state_gen ~csts:csts' false
		       (Closure.RedFlags.red_add_transparent betadeltaiota ts) env sigma (t,args) 
		     in if isConstruct t_o then whrec csts_o (t_o, stack_o@stack') else s,csts' in
	let _ = Timer.stop_timer name in
	result
      with exn -> 
	let _ = Timer.stop_timer name in
    ...
    |args, (Stack.Proj (n,m,p,_) :: stack'') -> (
      let name = "whd_bi_dz_fis BRANCH 3" in
      let _ = Timer.start_timer name in
      try 
	let result = let (t_o,stack_o),csts_o = whd_state_gen ~csts:csts' false
    ...
    |_, ((Stack.App _| Stack.Shift _|Stack.Update _|Stack.Cst _) :: _|[]) -> (
      let name = "whd_bi_dz_fis BRANCH 4" in
      let _ = Timer.start_timer name in
      try 
	let result = s,csts' in
	let _ = Timer.stop_timer name in
	result
      with exn -> 
	let _ = Timer.stop_timer name in
