...
let rec extract_instance_status_ORIG arg = 
  match arg with 
  | CUMUL -> add_type_status (IsSubType, IsSuperType)
  | CONV -> add_type_status (Conv, Conv)
and extract_instance_status arg = 
  let name = "extract_instance_status" in
  let _ = Timer.start_timer name in
  try 
    let result = extract_instance_status_ORIG arg in
    let _ = Timer.stop_timer name in
    result
  with
    exn ->
...
let rec default_core_unify_flags_ORIG () =
...
and default_core_unify_flags () =
  let name = "default_core_unify_flags" in
  let _ = Timer.start_timer name in
  try 
    let result = default_core_unify_flags_ORIG () in
    let _ = Timer.stop_timer name in
    result
  with
    exn ->
...
(* We set all conversion flags (no flag should be modified anymore)   *)
let rec default_unify_flags_ORIG () =
...
and default_unify_flags () =
  let name = "default_unify_flags" in
  let _ = Timer.start_timer name in
  try 
    let result = default_unify_flags_ORIG () in
    let _ = Timer.stop_timer name in
    result
  with
    exn ->
...
let rec set_no_delta_core_flags_ORIG flags = 
{ flags with
...
and set_no_delta_core_flags flags = 
  let name = "set_no_delta_core_flags" in
  let _ = Timer.start_timer name in
  try 
    let result = set_no_delta_core_flags_ORIG flags in
    let _ = Timer.stop_timer name in
    result
  with
    exn ->
...
let rec set_no_delta_flags_ORIG flags = 
{
...
and set_no_delta_flags flags = 
  let name = "set_no_delta_flags" in
  let _ = Timer.start_timer name in
  try 
    let result = set_no_delta_flags_ORIG flags in
    let _ = Timer.stop_timer name in
    result
  with
    exn ->
...
(* out of "simple apply" *)
let rec default_no_delta_core_unify_flags_ORIG () = 
{ (default_core_unify_flags ()) with
  modulo_delta = empty_transparent_state;
  check_applied_meta_types = false;
  use_pattern_unification = false;
  use_meta_bound_pattern_unification = true;
...
and default_no_delta_core_unify_flags () = 
  let name = "default_no_delta_core_unify_flags" in
  let _ = Timer.start_timer name in
  try 
    let result = default_no_delta_core_unify_flags_ORIG () in
    let _ = Timer.stop_timer name in
    result
  with
    exn ->
...
let rec default_no_delta_unify_flags_ORIG () =
...
and default_no_delta_unify_flags () =
  let name = "default_no_delta_unify_flags" in
  let _ = Timer.start_timer name in
  try 
    let result = default_no_delta_unify_flags_ORIG () in
    let _ = Timer.stop_timer name in
    result
  with
    exn ->
...
let rec elim_core_flags_ORIG sigma = 
{ (default_core_unify_flags ()) with
  modulo_betaiota = false;
  frozen_evars =
...
and elim_core_flags sigma = 
  let name = "elim_core_flags" in
  let _ = Timer.start_timer name in
  try 
    let result =  elim_core_flags_ORIG sigma in
    let _ = Timer.stop_timer name in
    result
  with
    exn ->
...
let rec elim_flags_evars_ORIG sigma =
...
and elim_flags_evars sigma =
  let name = "elim_flags_evars" in
  let _ = Timer.start_timer name in
  try 
    let result = elim_flags_evars_ORIG sigma in
    let _ = Timer.stop_timer name in
    result
  with
    exn ->
...
let rec elim_no_delta_core_flags_ORIG () = 
{ (elim_core_flags Evd.empty) with
  modulo_delta = empty_transparent_state;
  check_applied_meta_types = false;
  use_pattern_unification = false;
  modulo_betaiota = false;
}
and elim_no_delta_core_flags () = 
  let name = "elim_no_delta_core_flags" in
  let _ = Timer.start_timer name in
  try 
    let result = elim_no_delta_core_flags_ORIG () in
    let _ = Timer.stop_timer name in
    result
  with
    exn ->
...
let rec set_flags_for_type_ORIG flags = 
{ flags with
...
and set_flags_for_type flags = 
  let name = "set_flags_for_type" in
  let _ = Timer.start_timer name in
  try 
    let result = set_flags_for_type_ORIG flags in
    let _ = Timer.stop_timer name in
    result
  with
    exn ->
...
let rec use_metas_pattern_unification_ORIG flags nb l =
  !global_pattern_unification_flag && flags.use_pattern_unification
  || (Flags.version_less_or_equal Flags.V8_3 || 
      flags.use_meta_bound_pattern_unification) &&
     Array.for_all (fun c -> isRel c && destRel c <= nb) l
and use_metas_pattern_unification flags nb l =
  let name = "use_metas_pattern_unification" in
  let _ = Timer.start_timer name in
  try 
    let result = use_metas_pattern_unification_ORIG flags nb l in
    let _ = Timer.stop_timer name in
    result
  with
    exn ->
...
let rec expand_table_key_ORIG env arg = 
  match arg with
...
let rec expand_key_ORIG ts env sigma arg = 
  match arg with
...
let rec isAllowedEvar_ORIG flags c = 
  match kind_of_term c with
  | Evar (evk,_) -> not (Evar.Set.mem evk flags.frozen_evars)
  | _ -> false
and isAllowedEvar flags c = 
  let name = "isAllowedEvar" in
  let _ = Timer.start_timer name in
  try 
    let result = isAllowedEvar_ORIG flags c in
    let _ = Timer.stop_timer name in
    result
  with
    exn ->
...
let rec unify_0_ORIG env sigma = 
  unify_0_with_initial_metas (sigma,[],[]) true env
and unify_0 env sigma = 
  let name = "unify_0" in
  let _ = Timer.start_timer name in
  try 
    let result = unify_0_ORIG env sigma in
    let _ = Timer.stop_timer name in
    result
  with
    exn ->
...
let rec applyHead_ORIG env evd n c  =
...
let rec default_matching_core_flags_ORIG sigma =
  let ts = Names.full_transparent_state in {
...
and default_matching_core_flags sigma =
  let name = "default_matching_core_flags" in
  let _ = Timer.start_timer name in
  try
    let result = default_matching_core_flags_ORIG sigma in
    let _ = Timer.stop_timer name in 
    result 
  with
    exn -> 
...
and make_abstraction_core name0 (test,out) env sigma c ty occs check_occs concl =
  let name = "make_abstraction_core" in
  let _ = Timer.start_timer name in
  try
    let result = make_abstraction_core_ORIG name0 (test,out) env sigma c ty occs check_occs concl in
    let _ = Timer.stop_timer name in 
    result 
  with
    exn -> 
